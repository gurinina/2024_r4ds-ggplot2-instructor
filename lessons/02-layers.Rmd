...
```
  ...
  b. What if you were interested in a specific class, e.g. compact cars? How would you highlight that group using only an additional layer added to the plot below?
...
5.  a. In our proportion bar chart, we needed to set `group = 1`. Why? In other words, what is the problem with these two graphs?
...
**Ans-2.5.1.5a:**
  ...
  b. Replot the two plots below without using `group = 1` and use 'after_stat(count)' to calculate proportions instead of `after_stat(prop)'. Label the y-axis using percentages. Hint: check out the scales package that we loaded.
...
3.  What parameters to `geom_jitter()` control the amount of jittering? Add jittering to the following plot:
