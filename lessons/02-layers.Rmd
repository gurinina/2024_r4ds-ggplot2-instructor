...
Use the following ggplot to create 4 plots, `geom_line()`, `geom_boxplot()`, `geom_point()` and `geom_area()`. Include a title specifying the geometry for each.
...
4.  Recreate the R code necessary to generate and save the following graphs, and arrange them in a figure in 3 columns. Note that wherever a categorical variable is used in the plot, it's `drv`. Use the mathematical notation in one, and the function from `patchwork`, `plot_layout` in another for laying out the plot.
...
ggplot(mpg, aes(x = displ, y = hwy)) + 
  geom_point() + 
  facet_grid(drv ~ cyl) +
  theme(strip.text.x = element_text(size=12, face="bold",colour="blue",angle=75),strip.text.y = element_text(size=14, face="bold",colour="red"),
  strip.background = element_rect(colour="navy", fill="#CCCCFF", linewidth  = 2))
```
