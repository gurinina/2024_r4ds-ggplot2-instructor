...
>>>>>>> 3726d27 (Delete 01-vizualization.Rmd)
ggplot(mpg, aes(x = displ, y = hwy)) +
...
4.  Recreate the R code necessary to generate and save the following graph (\@ref(fig:fig-6-plots)), and arrange them in a figure with 2 columns. Note that wherever a categorical variable is used in the plot, it's `drv`. Use the mathematical notation in one, and the function from `patchwork`, `plot_layout` in another for laying out the plot.
...
<<<<<<< HEAD
(ggplot(mpg, aes(x = displ, y = hwy)) +
  geom_point()) + (ggplot(mpg, aes(x = displ, y = hwy)) +
