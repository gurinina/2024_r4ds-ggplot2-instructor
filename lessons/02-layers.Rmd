...
output: 
  xaringan::moon_reader:
    ...
    highlight: tango
    includes:
      after_body:  "scripta/collapse-output.html"
editor_options: 
...
<button type="btn btn-primary" class="collapsible">where</button>
<div class="content">ggplot(mpg, aes(x = hwy, y = displ)) +geom_point(color = "pink", shape = "triangle")</div>
...
The `r mpg` data frame is bundled with the ggplot2 package and contains `r nrow(mpg)` observations on `r mpg %>% distinct(model) %>% nrow()` car models.
...
.details{
```{r, echo = FALSE, details.summary = 'Ans: 3.'}
  
x = 'ggplot(mpg, aes(x = hwy, y = displ)) +
      geom_point(color = "pink", shape = "triangle")'
...
details::details(hooks$chunk(x,options))      
...
#### Ans. 2 ####
.details{
3.  A. What does the `stroke` aesthetic do?
    B. What point shapes does it work with?
    (hint: use `?geom_point`)
    
```{.details,  echo = FALSE, .details.summary = 'Ans: 3.'}
...
4.  Recreate the R code necessary to generate and save the following graphs, and arrange them in a figure.
