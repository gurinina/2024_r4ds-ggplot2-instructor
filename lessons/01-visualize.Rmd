# Data visualization {#sec-visualize}

```{r, eval = FALSE}
BiocManager::install(c("palmerpenguins","patchwork","ggthemes"))
```
...
We will also use the **palmerpenguins** package, which includes the `penguins` dataset containing body measurements for penguins on three islands in the Palmer Archipelago, and the ggthemes package, which offers a colorblind safe color palette.
...
`penguins` contains `r nrow(penguins)` observations collected and made available by Dr. Kristen Gorman and the Palmer Station, Antarctica LTER[^01-visualize-1].

[^01-visualize-1]: Horst AM, Hill AP, Gorman KB (2020).
...
### Ultimate goal {#data-visualize-data-visualize-data-visualize-sec-ultimate-goal}
...
### Adding aesthetics and layers {#data-visualize-data-visualize-data-visualize-sec-adding-aesthetics-layers}
...
## ggplot2 calls {#data-visualize-data-visualize-data-visualize-sec-ggplot2-calls}
...
The first argument of `facet_wrap()` is a formula[^01-visualize-2], which you create with `~` followed by a variable name.
The variable that you pass to `facet_wrap()` should be categorical.

[^01-visualize-2]: Here "formula" is the name of the thing created by `~`, not a synonym for "equation".
...
## Saving your plots {#data-visualize-data-visualize-data-visualize-sec-ggsave}
