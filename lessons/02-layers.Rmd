...
functions used in this chapter, first install the packages `patchwork`
and `ggridges` if you need to by uncommenting this code:
...
Then load the `tidyverse`, `scales` and `patchwork` packages by into
your session:
...
Note the use of `plot_layout` from the `patchwork` package to layout the
plots in two rows. You can also use mathematical symbols to layout
`ggplots`:
...
ordered aesthetic (`size` or `alpha`) is generally not a good idea
...
argument of your `geom` function (*outside* of `aes()`) instead of
relying on a variable mapping to determine the appearance. For example,
...
So far we have discussed aesthetics that we can map or set in a
scatterplot, when using a point geom. You can learn more about all
possible aesthetic mappings in the aesthetic specifications vignette at
<https://ggplot2.tidyverse.org/articles/ggplot2-specs.html>.

The specific aesthetics you can use for a plot depend on the geom you
use to represent the data. In the next section we dive deeper into
geoms.
...
```{r Ans:2.2.1.1}
ggplot(mpg,aes(x = hwy, y = displ)) + geom_point(shape = 17, color = "pink")
```
...
4.  What happens if you map an aesthetic to something other than a
    variable name, like `aes(color = displ < 5)`? Try it by adding color
...
a different geometric object, `geom`, to represent the data. The first
plot uses the point geom, and the plot on the second uses the smooth
geom, a smooth line fitted to the data.

To change the geom in your plot, change the geom function that you add
to `ggplot()`. For instance, to make the plots above, you can use the
following code:
...
defined locally in the geom layer or globally in the `ggplot()` layer.

However, not every aesthetic works with every geom. You could set the
shape of a point, but you couldn't set the "shape" of a line. If you
try, ggplot2 will silently ignore that aesthetic mapping. On the other
hand, you *could* set the `linetype` of a line.
...
whenever you map an aesthetic to a discrete variable (as in the
`linetype` example).
...
cars. The local data argument in `geom_point()` overrides the global
data argument in `ggplot()` for that layer only.
...
For example, the **ggridges** package
([https://wilkelab.org/ggridges](https://wilkelab.org/ggridges/){.uri})
is useful for making ridgeline plots, which can be useful for
visualizing the density of a numerical variable for different levels of
a categorical variable.

In the following plot not only did we use a new geom
(`geom_density_ridges()`), but we have also mapped the same variable to
multiple aesthetics (`drv` to `y`, `fill`, and `color`) as well as set
an aesthetic (`alpha = 0.5`) to make the density curves transparent.
...
1.a. What geom would you use to draw a line chart? A boxplot? A
...
b.  Use the following ggplot to create 4 plots, `geom_line()`,
    `geom_boxplot()`, `geom_point()` and `geom_area()`. Include a title
    specifying the geometry for each.
...
4.  Recreate the R code necessary to generate and save the following
    graph \@ref(fig:6-plots) and arrange them in a figure with 2
    columns. Note that wherever a categorical variable is used in the
    plot, it's `drv`. Use the mathematical notation in one, and the
    function from `patchwork`, `plot_layout` in another for laying out
    the plot.
...
The first argument in `facet_grid` determines the faceting by column,
the second by row. By default each of the facets share the same scale
and range for x and y axes.
...
2.  Run the following code and facet on drv vs cyl. What do the empty
    cells mean?
...
a.  What are the advantages to using faceting instead of the color
    aesthetic? What are the disadvantages?
...
6.  Which of the following plots makes it easier to compare engine size
    (`displ`) across cars with different drive trains? What does this
    say about when to place a faceting variable across rows or columns?
...
8.  Modify the facet label in the following plot so that it has a
    background color of `navy` with a `red` outline and `white` text.
...
\@ref(fig:fig-vis-stat-bar) shows how this process works with
`geom_bar()`.
...
ggplot2 provides more than 20 stats for you to use. Each stat is a
function, so you can get help in the usual way, e.g., `?stat_bin`.
...
2.  
    ...
    a.  What does `geom_col()` do? How is it different from
        `geom_bar()`?

**Ans-2.5.1.1:**

b.  Plot mpg and count the classes in drv. Compare `geom_bar` to
    `geom_col`.
...
3.  Most geoms and stats come in pairs that are almost always used in
    ...
    (Hint: Read through the documentation.). Write the stat used for
    `geom_bar()`,`geom_smooth()` and `geom_boxplot()`.
...
5.  
    ...
    a.  In our proportion bar chart, we needed to set `group = 1`. Why?
        In other words, what is the problem with these two graphs?
        Replot the two plots below using `group = 1`.
...
b.  Replot the two plots below without using `group = 1` and use
    'after_stat(count)' to calculate proportions instead of
    \`after_stat(prop)'. Label the y-axis using percentages. Hint: check
    out the scales package that we loaded.
...
5.  What's the default position adjustment for `geom_boxplot()`? Create
    ...
    the default position using the following ggplot:
