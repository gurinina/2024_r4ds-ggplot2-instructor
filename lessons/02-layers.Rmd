...
In [the last chapter][sec-visualize], you learned much more than just how to make scatterplots, bar charts, and boxplots.
...
To access the datasets, help pages, and functions used in this chapter, load the tidyverse and the patchwork packages by running this code:
...
ggplot(mpg, aes(x = displ, y = hwy, color = class)) +
  geom_point() + ggplot(mpg, aes(x = displ, y = hwy, shape = class)) +
  geom_point() + plot_layout(nrow = 2)
```
...
ggplot(mpg, aes(x = displ, y = hwy, size = class)) +
  geom_point() + ggplot(mpg, aes(x = displ, y = hwy, alpha = class)) +
  geom_point() +   plot_layout(ncol = 2)
```
...
4.  What happens if you map an aesthetic to something other than a variable name, like `aes(color = displ < 5)`? Try it. Note, you'll also need to specify x and y.
...
You could set the shape of a point, but you couldn't set the "shape" of a line. If you try, ggplot2 will silently ignore that aesthetic mapping. On the other hand, you *could* set the linetype of a line.
...
If you place mappings in a geom function, ggplot2 will treat them as local mappings for the layer. It will use these mappings to extend or overwrite the global mappings *for that layer only*.
 his makes it possible to display different aesthetics in different layers.
...
Use the following ggplot to create 4 plots, `geom_line()`, `geom_boxplot()`, `geom_histogram()` and `geom_area()`.
...
Rerun the plot with `show-legend = TRUE`
...
4.  Recreate the R code necessary to generate and save the following graphs, and arrange them in a figure. Note that wherever a categorical variable is used in the plot, it's `drv`.
...
What if you were interested in a specific class, e.g. compact cars? How would highlight that group only using an additional layer added to the plot below?
...
Use the ggplot below and compare adding a `geom_bar` layer to a `geom_col` to count the classes in drv.
...
```{r}
    ggplot(mpg, aes(x = displ, y = hwy)) +
      geom_point() + 
    ggplot(mpg, aes(x = displ, y = hwy)) +
      geom_point(position = "identity") +            plot_layout(ncol = 2)
```
...
5.  What's the default position adjustment for `geom_boxplot()`? Create a visualization of the `mpg` dataset that demonstrates it by adding different positions using the following ggplot:

```{r}
g <- ggplot(mpg, aes(x = cty, y = displ)) 
```
